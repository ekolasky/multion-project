...
def research_company(name, max_sources=4, max_steps=12, max_num_repeats=1, update_callback=None):
    ...
    try:
        ...
        update_callback("Agent's message does not fit template. Rerunning research process.")
        print(e)
        # If there's an issue with the output, rerun function up to number of repeats
        if num_repeats < max_num_repeats:
            research_company(
...
def research_loop(prompt, name, max_sources=4, max_steps=12, update_callback=None):
    ...
    # Initiate loop
    i = 0
    hard_max_steps = max_steps + 2
    visited_links = []
    response = None
    while i <= hard_max_steps and (not response or response.status == 'CONTINUE'):
        i += 1
        cmd = prompt

        # Have agent return summary if it hasn't returned after max_steps
        if (i == max_steps):
            cmd = f'Now please return your summary of information about {name}. Do not take any more actions.'
        elif (i >= max_steps):
            cmd = f'Return your summary of information about {name} immediately. If you take any more actions, you will be shut down.'

        # If the agent has just read its nth webpage and is returning to Google have it output summary
        elif (len(visited_links) >= max_sources and response and response.url[:23] != "https://www.google.com/"):
        ...
        if (update_callback and i <= max_steps):
